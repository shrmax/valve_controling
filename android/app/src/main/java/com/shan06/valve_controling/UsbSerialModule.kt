...
fun sendCommand(command: String, promise: Promise) {
    ...
    try {
        // Clear any stale data in the read buffer
        val flushBuffer = ByteArray(port.readEndpoint.maxPacketSize)
        while (port.read(flushBuffer, 200) > 0) {
            // Discarding stale data
        }

        Log.d("UsbSerialModule", "Sending command: $command")
        ...
        var bytesRead: Int
        do {
            bytesRead = port.read(responseBuffer, 200)
            if (bytesRead > 0) {
                val chunk = responseBuffer.copyOf(bytesRead)
                responseBuilder.append(String(chunk, Charsets.UTF_8))
                // Simple check for end of message, e.g., a newline character
        ...
        Log.d("UsbSerialModule", "Received response: $response")

        if (response.isNotEmpty()) {
            promise.resolve(response)
        } else {
            promise.reject("NO_RESPONSE", "No response from device within timeout.")
        }
    } catch (e: Exception) {
        Log.e("UsbSerialModule", "Error during sendCommand", e)
        promise.reject("SEND_COMMAND_ERROR", "Error sending command: ${e.localizedMessage}", e)
    }
}
